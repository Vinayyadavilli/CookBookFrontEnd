...
export function NavBar({ onNavigate, currentScreen, isPremium }) {
    ...
    <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, height: 72, display: 'flex', alignItems: 'center', padding: '0 32px', background: scrolled ? 'rgba(250,250,248,0.95)' : 'rgba(250,250,248,0.85)', backdropFilter: 'blur(16px)', borderBottom: scrolled ? `1px solid ${C.border}` : '1px solid transparent', transition: 'all 0.25s' }}>
      ...
      <div style={{ flex: 1, display: 'flex', justifyContent: 'center', gap: 4 }}>
        {[
          ...
          <button key={s} onClick={() => onNavigate(s)}
            style={{ fontSize: 13, fontWeight: 500, color: currentScreen === s ? C.primary : C.ink2, padding: '6px 14px', borderRadius: 8, border: 'none', background: currentScreen === s ? C.primaryLight : 'transparent', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s', display: 'flex', alignItems: 'center', gap: 5 }}>
            {label}
            {isProOnly && !isPremium && (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => onNavigate('profile')}>
          <img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=80&h=80&fit=crop&face" alt="Avatar" style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover', border: `2px solid ${isPremium ? C.gold : C.primary}` }} />
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: C.ink }}>Hi, Vinay 👋</div>
